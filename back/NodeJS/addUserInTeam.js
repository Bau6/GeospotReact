...
function addPlayer(params, res) {
    let {team_id, user_id, sport_id} = params; // Деструктурируем только нужные поля из params
    ...
    connection.query("SELECT * FROM ?? WHERE user_id = ? AND team_id = ?", [TEAM, user_id, team_id], (selectErr, selectResult) => {
        if (selectErr || teamData.team_id === "" || teamData.user_id === "") {
            ...
            connection.query("SELECT * FROM ?? WHERE user_id = ?", ["user_sport_qualification", user_id], (selectErr, selectQuals) => {
                if (selectErr) {
                    console.log(selectErr);
                    ...
                    });} else {res.json({error: 'Недоступная квалификация!'});}
                }
            });
        }
    });
